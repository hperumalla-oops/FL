...
#     # Skip rows before `start_row`, and read only `num_rows`
#     df = pd.read_csv(path, skiprows=range(1, start_row + 1), nrows=num_rows)
#     X = df.iloc[:, :-1].values
#     y = df.iloc[:, -1].values
...
# = load_data("client1_data.csv")
path = r"C:\Users\hperu\OneDrive\Desktop\fl\mitbih_train.csv"

num_rows = args.size // args.total
start = (args.nth - 1) * num_rows 

df = pd.read_csv(path, skiprows=range(1, start  + 1), nrows=num_rows)
X = df.iloc[:, :-1].values
y = df.iloc[:, -1].values
